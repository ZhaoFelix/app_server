<template lang="html">
  <div class="main">
      ...
      <el-input
          ...
          :rows="2"
          placeholder="请输入文章简介"
          v-model = "preview" style="margin-top:10px;">
      </el-input>
      <el-row class="my-row">
        <el-col :span="4">请选择前端cell的类型：</el-col>
        <el-col :span="5">
...
export default {
  ...
  methods: {
    submit() {
      ...
      if (this.blogTitle && this.blogImageUrl && this.content && this.radio && this.preview) {
        var tagStr = "";
        ...
        if (id != 0) {
          ...
          this.$axios
            .post(
              "http://www.bedeveloper.cn:3000/list/update",
              qs.stringify({
                _id: id,
        ...
        this.$axios
          .post(
            "http://www.bedeveloper.cn:3000/list/add",
            qs.stringify({
              title: this.blogTitle,
              content: this.content,
              imageUrl: this.blogImageUrl,
  ...
  created: function() {
    ...
    if (id == 0) {
      ...
      this.$axios
        .post(
          "http://www.bedeveloper.cn:3000/list/querySingle",
          qs.stringify({
            id: id
          })
        )
    ...
    //获取标签
    this.$axios
      .get("http://www.bedeveloper.cn:3000/other/tag/query")
      .then(function(response) {
        var re = response.data;
        if (re.code == 0) {
